...
def scatterLineBarSpecific_GUIWindow(plotType,dataType):
    ...
    for axis,i in zip(constantAxes,range(len(constantAxes))):
        ...
        if 'Y' in axis:
            if plotType == 'ordered':
                radioValues = pickle.load(open('gui-radioVals.pkl','rb'))
    ...
    linThresholdValues = {}
    axisTitleValues = {'X Axis':initial_name,'Y Axis':initial_name}
    ...
    class GUIButtons4(object):
        def OKradiotext4(self, event):
            ...
            with open('gui-plotOptions.pkl','wb') as f:
