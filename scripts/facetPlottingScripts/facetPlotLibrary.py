...
def levelsInFigure_GUIWindow(labelDict):
    ...
    class GUIButtons(object):
        def OKcheck1(self, event):
            ...
            plt.close()
            with open('gui-wfBool.pkl','wb') as f:
...
def levelValuesInFigure_GUIWindow(labelDict):
    ...
    class GUIButtons2(object):
        def OKcheck2(self, event):
            ...
            plt.close()
            with open('gui-svBoolList.pkl','wb') as f:
...
def levelsToPlottingParameters_GUIWindow(labelDict,plotType,subPlotType,dataType):
    if plotType == 'categorical':
        ...
        if subPlotType == 'heatmap':
            parameterTypes = ['Row','Column','X Axis Values']
    else:
        parameterTypes = ['Marker','Color','Size','Row','Column','X Axis Values']
    if plotType == '3d' or (plotAgainstAnother and subPlotType in ['scatter','bar']):
        if plotType == '3d':
            parameterTypes+=['Y Axis Values']
        else:
            if subPlotType == 'bar':
                parameterTypes+=['None']
            else:
                parameterTypes+=['Y Axis Values','None']

    withinFigureBoolean = pickle.load(open('gui-wfBool.pkl','rb'))
    selectedLevels = []
    ...
    if plotAgainstAnother:
        selectedLevels = selectedLevels[:-1]
        svboolList = pickle.load(open('gui-svBoolList.pkl','rb'))
        for boolVal,levelVal in zip(svboolList,labelDict['DeconvolutionParameters']):
            if boolVal and plotType != 'categorical':
                selectedLevels.append(levelVal)
    ...
    class GUIButtons3(object):
        def OKradio3(self, event):
            ...
            plt.close()
            with open('gui-radioVals.pkl','wb') as f:
...
def produceSubsettedDataFrames(df):
    ...
    withinFigureBoolean = pickle.load(open('gui-wfBool.pkl','rb'))
    specificValueBooleanList = pickle.load(open('gui-svBoolList.pkl','rb'))
...
def plotFacetedFigures(folderName,plotType,subPlotType,dataType,subsettedDfList,subsettedDfListTitles,figureParameters,levelsPlottedIndividually,useModifiedDf,fulldf):
    legendParameterToLevelNameDict = pickle.load(open('gui-radioVals.pkl','rb'))
    plotOptions = pickle.load(open('gui-plotOptions.pkl','rb'))

    subprocess.run(['rm','gui-radioVals.pkl'])
    subprocess.run(['rm','gui-plotOptions.pkl'])
    subprocess.run(['rm','gui-wfBool.pkl'])
    subprocess.run(['rm','gui-svBoolList.pkl'])
    ...
    for subsettedDf,subsettedDfTitle in zip(subsettedDfList,subsettedDfListTitles):
        if plotAgainstAnother:
            subsettedDf = subsettedDf.unstack('DeconvolutionParameters')
            subsettedDf.columns.name = ''
            print(subsettedDf)
        ...
        for parameter in legendParameterToLevelNameDict:
            ...
            if parameter == 'Color':
                ...
                kwargs['hue_order'] = list(pd.unique(subsettedDf.index.get_level_values(currentLevel)))
            elif parameter == 'Size':
                ...
                if len(plotOptions['axisTitles']['X Axis']) > 1 and not plotAgainstAnother:
                    subsettedDf.index.set_names(plotOptions['axisTitles']['X Axis'],level=currentLevel,inplace=True)
                    kwargs['x'] = plotOptions['axisTitles']['X Axis']
                else:
...
def plotSubsettedFigure(subsettedDf,plottingDf,kwargs,facetgridkwargs,plotType,subPlotType,dataType,fullTitleString,plotOptions,subsettedDfTitle):
    ...
    #2d plots: categorical plots (bar, point), ordered plots (line,scatter)
        ...
        if len(kwargs['hue_order']) > 8:
            sns.set_palette(sns.color_palette("magma", len(kwargs['hue_order'])))
        if plotType == 'categorical':
    ...
    if 'row' not in kwargs and 'col' not in kwargs and dataType:
    ...
    #Save figure
    if len(fullTitleString) > 251:
        fullTitleString = fullTitleString[:251]
    os.chdir('kineticFeatureFigures')
    plt.savefig(fullTitleString+'.png',bbox_inches='tight')
    os.chdir('..')
    #fg.fig.savefig('kineticFeatureFigures/'+fullTitleString+'.png',bbox_inches='tight')
...
def facetPlottingGUI(df,plotType,subPlotType,dataType):
    ...
    if plotType in ['categorical','ordered','1d']:
        scatterLineBarSpecific_GUIWindow(plotType,dataType)
    elif subPlotType == 'heatmap':
        heatmapSpecific_GUIWindow(plotType,dataType)
    else:
        pass
